        ...
        <div className="space-y-8 animate-fadeIn">
            ...
            <div className="flex justify-between items-center">
                ...
                {(isPastor || isMinistro) && (
                    <button
                        ...
                        className="bg-slate-900 text-white px-5 py-3 rounded-xl flex items-center shadow-lg hover:bg-black transition-all btn-hover cursor-pointer"
                    >
                ...
                <div className="bg-white p-6 rounded-[2rem] shadow-card border border-sky-100 animate-slideUp">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
                        <input
                            ...
                            className="p-3 bg-slate-50 rounded-xl border border-slate-200 outline-none focus:ring-2 focus:ring-blue-200"
                        ...
                        <div className="col-span-1 md:col-span-2">
                            ...
                            <select
                                className="w-full p-3 bg-slate-50 rounded-xl border border-slate-200 outline-none focus:ring-2 focus:ring-blue-200 font-bold text-slate-700"
                    ...
                    <div className="flex justify-end gap-2">
                        <button onClick={() => setIsCreating(false)} className="px-4 py-2 text-slate-500 hover:bg-slate-100 rounded-xl transition-colors cursor-pointer">Cancelar</button>
                        <button onClick={handleCreateTrip} disabled={!newTripGroupId} className="px-6 py-2 bg-slate-900 text-white font-bold rounded-xl hover:bg-black transition-colors disabled:opacity-50 cursor-pointer">Crear Viaje</button>
                    </div>
                </div>
            )}
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                ...
                {visibleTrips.map(trip => {
                    ...
                    return (
                        <div key={trip.id} className="bg-white rounded-[2rem] p-6 shadow-card border border-slate-50 flex flex-col h-full relative overflow-hidden group">
                            {/* Status Badge */}
                            <div className={`absolute top-0 right-0 px-4 py-2 rounded-bl-2xl text-[10px] font-extrabold uppercase tracking-wider ${trip.status === 'APROBADO' ? 'bg-emerald-500 text-white' :
                                                ...
                                                return (
                                                    <div key={`pend-${idx}`} className="bg-white p-2 rounded-lg flex justify-between items-center shadow-sm">
                                                        ...
                                                        <div className="flex gap-1">
                                                            {(isPastor || isMinistro) ? (
                                                                <>
                                                                    <button onClick={() => handleApproveParticipant(trip.id, p.memberId, true)} className="p-1 bg-emerald-100 text-emerald-600 rounded hover:bg-emerald-200 cursor-pointer"><Check className="w-3 h-3" /></button>
                                                                    <button onClick={() => handleApproveParticipant(trip.id, p.memberId, false)} className="p-1 bg-red-100 text-red-600 rounded hover:bg-red-200 cursor-pointer"><X className="w-3 h-3" /></button>
                                                                </>
                                                            ) : (
                            ...
                            <div className="flex gap-2">
                                ...
                                {(trip.status === 'PLANIFICACION' || trip.status === 'EN_REVISION') && canPropose(trip) && !isMember && (
                                    <button
                                        ...
                                        className={`flex-1 py-3 border-2 font-bold rounded-xl transition-colors flex justify-center items-center gap-2 text-xs uppercase tracking-wide cursor-pointer ${isPastor
                                ...
                                {trip.status === 'APROBADO' && !isMember && (
                                    <button
                                        ...
                                        className="flex-1 py-3 bg-blue-600 text-white font-bold rounded-xl hover:bg-blue-700 transition-colors flex justify-center items-center gap-2 text-xs uppercase tracking-wide shadow-lg"
                                    >
            ...
            {isProposeOpen && (
                <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/40 backdrop-blur-sm p-4 animate-fadeIn">
                    <div className="bg-white w-full max-w-md rounded-[2rem] shadow-2xl p-8 relative h-[80vh] flex flex-col border border-white/50">
                        <button onClick={() => setIsProposeOpen(false)} className="absolute top-6 right-6 p-2 bg-slate-50 rounded-full hover:bg-slate-100 text-slate-400 cursor-pointer"><X className="w-5 h-5" /></button>
                        ...
                        <div className="relative mb-4">
                            <input
                                className="w-full pl-10 pr-4 py-3 bg-slate-50 rounded-xl border-none focus:ring-2 focus:ring-blue-200 font-medium"
                        ...
                        <div className="flex-1 overflow-y-auto space-y-2 mb-4 pr-2 custom-scrollbar">
                            {getFilteredMembersForProposal().map(m => (
                                <div
                                    ...
                                    className={`flex items-center gap-3 p-3 rounded-xl cursor-pointer transition-colors border ${selectedMemberId === m.id ? 'bg-blue-50 border-blue-400' : 'bg-white border-slate-100 hover:border-slate-300'}`}
                                >
                                    ...
                                    {selectedMemberId === m.id && <Check className="w-5 h-5 text-blue-500 ml-auto" />}
                                </div>
                            ))}
                        ...
                        <div className="pt-4 border-t border-slate-100">
                            ...
                            <div className="grid grid-cols-2 gap-2 mb-4">
                                {['INTERCESOR', 'MUSICO', 'EVANGELISTA', 'APOYO'].map((role) => (
                                    <button
                                        ...
                                        className={`py-2 px-3 rounded-lg text-[10px] font-bold border transition-colors uppercase cursor-pointer ${selectedRole === role
                            ...
                            <button
                                ...
                                className="w-full py-3 bg-slate-900 text-white rounded-xl font-bold hover:bg-black transition-colors disabled:opacity-50 shadow-lg cursor-pointer"
                            >
                ...
                <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/40 backdrop-blur-sm p-4 animate-fadeIn">
                    <div className="bg-white w-full max-w-sm rounded-[2rem] shadow-2xl p-8 relative border border-white/50 text-center">
                        ...
                        <div className="flex gap-3">
                            <button onClick={() => setTripToLock(null)} className="flex-1 py-3 bg-slate-100 text-slate-600 font-bold rounded-xl hover:bg-slate-200 cursor-pointer">Cancelar</button>
                            <button onClick={confirmLockTrip} className="flex-1 py-3 bg-slate-800 text-white font-bold rounded-xl hover:bg-black shadow-lg cursor-pointer">Confirmar</button>
                        </div>
                    </div>
                </div>
            )}
            ...
            {executionTrip && (
                <div className="fixed inset-0 z-[60] flex items-center justify-center bg-slate-900/40 backdrop-blur-sm p-4 animate-fadeIn">
                    <div className="bg-white w-full max-w-md rounded-[2rem] shadow-2xl p-8 relative h-[80vh] flex flex-col border border-white/50">
                        <button onClick={() => setExecutionTrip(null)} className="absolute top-6 right-6 p-2 bg-slate-50 rounded-full hover:bg-slate-100 text-slate-400 cursor-pointer"><X className="w-5 h-5" /></button>
                        <div className="mb-4">
                                ...
                                return (
                                    <div key={p.memberId} className="flex items-center justify-between p-3 bg-white border border-slate-100 rounded-xl shadow-sm">
                                        ...
                                        <button
                                            ...
                                            className={`w-8 h-8 rounded-lg flex items-center justify-center transition-colors cursor-pointer ${p.attended ? 'bg-emerald-500 text-white shadow-lg' : 'bg-slate-100 text-slate-300'}`}
                                        >
                        ...
                        <button onClick={() => setExecutionTrip(null)} className="mt-4 w-full py-3 bg-slate-800 text-white font-bold rounded-xl hover:bg-black transition-colors cursor-pointer">
                            Finalizar Control
                        </button>
                    </div>
                </div>

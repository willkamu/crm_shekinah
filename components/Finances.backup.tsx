        ...
        <div className="space-y-8 animate-fadeIn">
            ...
            <div className="flex border-b border-slate-200 overflow-x-auto no-scrollbar no-print">
                <button
                    ...
                    className={`px-6 py-4 text-sm font-bold border-b-2 transition-colors whitespace-nowrap cursor-pointer ${activeTab === 'TRANSACCIONES' ? 'border-emerald-500 text-emerald-600' : 'border-transparent text-slate-400 hover:text-slate-600'}`}
                >
                    Registro Diario
                </button>
                <button
                    onClick={() => setActiveTab('REPORTES')}
                    className={`px-6 py-4 text-sm font-bold border-b-2 transition-colors whitespace-nowrap cursor-pointer ${activeTab === 'REPORTES' ? 'border-emerald-500 text-emerald-600' : 'border-transparent text-slate-400 hover:text-slate-600'}`}
                >
                    Cierre Mensual
                </button>
                {currentUser.role === 'PASTOR_PRINCIPAL' && (
                    <button
                        ...
                        className={`px-6 py-4 text-sm font-bold border-b-2 transition-colors whitespace-nowrap cursor-pointer ${activeTab === 'ANALISIS' ? 'border-emerald-500 text-emerald-600' : 'border-transparent text-slate-400 hover:text-slate-600'}`}
                    >
                        Análisis
                    </button>
                )}
                ...
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 no-print">
                    ...
                    <div className="bg-white p-6 md:p-8 rounded-[2.5rem] shadow-card border border-slate-50 h-fit">
                        ...
                        <form onSubmit={handleSubmitTransaction} className="space-y-6">
                            ...
                            <div className="grid grid-cols-3 gap-3">
                                {['Diezmo', 'Ofrenda', 'Honra Especial'].map(t => (
                                    <button
                                        ...
                                        className={`py-3.5 rounded-2xl text-xs md:text-sm font-bold transition-all btn-hover cursor-pointer ${type === t
                            ...
                            <button type="submit" className="w-full bg-emerald-500 text-white py-4 rounded-2xl font-bold hover:bg-emerald-600 shadow-glow transition-all btn-hover flex justify-center items-center gap-2 text-lg cursor-pointer">
                                        ...
                                        return (
                                            <li key={tx.id} className="p-5 flex justify-between items-center hover:bg-slate-50 transition-colors cursor-default group">
                                                <div className="flex items-center gap-4">
                                                    <div className={`p-3.5 rounded-2xl transition-colors ${tx.tipo === 'Diezmo'
                        ...
                        <div className="bg-gradient-to-r from-slate-800 to-slate-900 rounded-[2.5rem] p-8 text-white relative overflow-hidden">
                            <div className="relative z-10 flex flex-col md:flex-row justify-between items-center gap-6">
                                ...
                                <button
                                    ...
                                    className="bg-emerald-500 hover:bg-emerald-400 text-white px-8 py-4 rounded-2xl font-bold shadow-glow flex items-center gap-3 transition-transform hover:scale-105 cursor-pointer"
                                >
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                        {visibleReports.map(report => (
                            <div key={report.id} className="bg-white p-6 rounded-[2.5rem] shadow-card border border-slate-50 flex flex-col relative group hover:shadow-lg transition-all">
                                <button onClick={() => handlePrintReport(report)} className="absolute top-6 right-6 p-2 text-slate-300 hover:text-slate-600 transition-colors hidden group-hover:block cursor-pointer" title="Imprimir Reporte">
                                ...
                                <div className="flex justify-between items-start mb-4">
                                    ...
                                    <span className={`px-3 py-1 rounded-full text-[10px] font-bold uppercase ${report.status === 'RECIBIDO' ? 'bg-emerald-100 text-emerald-600' : 'bg-amber-100 text-amber-600'
                                        }`}>
                                        {report.status}
                                    </span>
                                </div>
                                ...
                                <div className="mt-auto flex gap-2">
                                    ...
                                    {currentUser.role === 'PASTOR_PRINCIPAL' && report.status !== 'RECIBIDO' && (
                                        <button
                                            ...
                                            className="flex-1 py-3 bg-emerald-500 text-white font-bold rounded-xl text-xs hover:bg-emerald-600 flex justify-center items-center gap-2 cursor-pointer"
                                        >
                ...
                <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/40 backdrop-blur-sm p-4 animate-fadeIn no-print">
                    <div className="bg-white w-full max-w-sm rounded-[2.5rem] shadow-2xl p-8 relative border border-white/50 text-center">
                        ...
                        <button
                            ...
                            className="w-full py-3 bg-emerald-500 text-white font-bold rounded-xl hover:bg-emerald-600 disabled:opacity-50 disabled:cursor-not-allowed shadow-glow transition-all cursor-pointer"
                        >
                            Confirmar Envío
                        </button>
                    </div>
                ...
                <div className="bg-white p-8 rounded-[2.5rem] shadow-card border border-slate-50 relative no-print">
                    <div className="flex justify-between items-center mb-6">
                        ...
                        <button
                            ...
                            className="bg-emerald-50 text-emerald-600 px-4 py-2 rounded-xl text-xs font-bold flex items-center gap-2 hover:bg-emerald-100 transition-colors cursor-pointer"
                        >
                ...
                <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/80 backdrop-blur-md p-4 animate-fadeIn no-print" onClick={() => setViewEvidenceUrl(null)}>
                    <div className="relative max-w-2xl w-full" onClick={e => e.stopPropagation()}>
                        <button
                            ...
                            className="absolute -top-12 right-0 text-white hover:text-slate-300 transition-colors cursor-pointer"
                        >
                            <X className="w-8 h-8" />
                        </button>

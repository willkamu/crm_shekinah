                ...
                <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 mb-6">
                    <div>
                        ...
                        <p className="text-slate-500 text-sm mt-1">
                            Días habilitados para esta sede: <span className="font-bold text-blue-600">{relevantDays}</span>
                        </p>
                    </div>

                    <div className="flex flex-col items-end">
                        <p className="text-sm text-slate-500">Total Presentes</p>
                        <p className="text-3xl font-bold text-blue-600">{presentCount} <span className="text-lg text-slate-300">/ {visibleMembers.length}</span></p>
                    </div>
                </div>
                ...
                <div className="flex gap-4 mb-6">
                    <div className="relative flex-1">
                        <select
                            ...
                            className="w-full pl-10 pr-4 py-3 bg-slate-50 border border-slate-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-blue-500 appearance-none font-medium text-slate-700"
                        >
                            {MOCK_EVENTS.map(e => (
                    ...
                    <button
                        ...
                        className="whitespace-nowrap px-6 py-3 bg-slate-900 text-white font-bold rounded-xl hover:bg-black transition-colors shadow-lg"
                    >
                        Marcar Todos
                    </button>
                </div>
                        ...
                        return (
                            <div key={member.id} className="py-3 flex items-center justify-between group">
                                ...
                                <div
                                    ...
                                    className={`w-14 h-8 flex items-center rounded-full p-1 cursor-pointer transition-all duration-300 ${isPresent ? 'bg-emerald-500 shadow-emerald-200 shadow-lg' : 'bg-slate-200'}`}
                                >

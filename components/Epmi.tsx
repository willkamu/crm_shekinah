...
const Epmi: React.FC = () => {
    ...
    // PERMISSION CHECK
    // PERMISSION CHECK
    const canGraduate = ['PASTOR_PRINCIPAL', 'PASTOR_GENERAL', 'PASTORA_GENERAL', 'PASTOR_EJECUTIVO'].includes(currentUser.role);
            ...
            <div className="bg-white rounded-[2rem] border border-slate-200 shadow-xl overflow-hidden mb-8">
                                ...
                                return (
                                    <tr key={student.id} className="hover:bg-slate-50/50 transition-colors group">
                                        ...
                                        <td className="p-4 text-right flex justify-end gap-2 sticky right-0 bg-white z-10 border-l border-slate-50 group-hover:bg-slate-50/50">
                                            <button onClick={() => handleEditStudent(student)} className="p-2 text-slate-300 hover:text-brand-blue rounded-lg hover:bg-slate-100 cursor-pointer" title="Editar">
                                                <Edit2 className="w-4 h-4" />
                                            </button>
                                            <button
                                                onClick={() => handleOpenGrading(student)}
                                                className="px-3 py-1.5 bg-slate-100 text-brand-blue font-bold text-xs rounded-lg hover:bg-brand-soft cursor-pointer"
                                            >
                                                Calificar
                                            </button>
                                            {canGraduate && (
                                                <button
                                                    ...
                                                    className="px-3 py-1.5 bg-emerald-50 text-emerald-600 font-bold text-xs rounded-lg hover:bg-emerald-100 cursor-pointer"
                                                >
                                                    Promover
                                                </button>
                                            )}
                ...
                <div className="md:hidden space-y-4 p-4">
                    ...
                    {students.map(student => {
                        const member = members.find(m => m.id === student.memberId);
                        return (
                            <div key={student.id} className="bg-white p-5 rounded-2xl border border-slate-100 shadow-sm">
    ...
    return (
        <div className="space-y-8 animate-fadeIn">
            ...
            <div className="flex items-center gap-6">
                <div className="bg-gradient-to-br from-blue-600 to-slate-900 p-5 rounded-2xl shadow-lg shadow-blue-200 text-white">
                    <GraduationCap className="w-10 h-10" />
                </div>
                <div>
                    <h2 className="text-4xl font-extrabold text-slate-900 tracking-tight mb-2">Escuela EPMI</h2>
                    <p className="text-slate-500 text-lg">Preparación Ministerial Internacional</p>
                </div>
            </div>
            ...
            <div className="flex border-b border-slate-200 overflow-x-auto no-scrollbar">
                <button
                    ...
                    className={`px-8 py-5 text-sm font-bold border-b-2 transition-all whitespace-nowrap cursor-pointer ${activeTab === 'CANDIDATOS' ? 'border-blue-600 text-blue-800' : 'border-transparent text-slate-400 hover:text-slate-600 hover:bg-slate-50'}`}
                >
                    Candidatos
                </button>
                <button
                    onClick={() => setActiveTab('AULA')}
                    className={`px-8 py-5 text-sm font-bold border-b-2 transition-all whitespace-nowrap cursor-pointer ${activeTab === 'AULA' ? 'border-blue-600 text-blue-800' : 'border-transparent text-slate-400 hover:text-slate-600 hover:bg-slate-50'}`}
                >
                    Aula Virtual
                </button>
                <button
                    onClick={() => setActiveTab('SERVICIO')}
                    className={`px-8 py-5 text-sm font-bold border-b-2 transition-all whitespace-nowrap cursor-pointer ${activeTab === 'SERVICIO' ? 'border-blue-600 text-blue-800' : 'border-transparent text-slate-400 hover:text-slate-600 hover:bg-slate-50'}`}
                >
                    Año de Servicio
                </button>
                <button
                    onClick={() => setActiveTab('GRADUADOS')}
                    className={`px-8 py-5 text-sm font-bold border-b-2 transition-all whitespace-nowrap cursor-pointer ${activeTab === 'GRADUADOS' ? 'border-blue-600 text-blue-800' : 'border-transparent text-slate-400 hover:text-slate-600 hover:bg-slate-50'}`}
                >
                    Graduados
                </button>
            </div>
                ...
                <div className="space-y-6">
                    <div className="flex justify-between items-center bg-blue-50 p-6 rounded-2xl border border-blue-100">
                        ...
                        <button
                            ...
                            className="bg-white text-blue-700 px-5 py-2.5 rounded-xl text-xs font-bold border border-blue-200 hover:bg-blue-50 shadow-sm flex items-center gap-2 transition-all cursor-pointer"
                        >
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                        {potentialCandidates.length === 0 ? (
                            ...
                            potentialCandidates.map(candidate => (
                                <div key={candidate.id} className="bg-white p-6 rounded-2xl border border-slate-100 shadow-sm flex flex-col justify-between hover:shadow-xl transition-all relative group">
                                    <button
                                        onClick={() => handleDiscardCandidate(candidate.id)}
                                        className="absolute top-4 right-4 p-2 text-slate-300 hover:text-red-500 hover:bg-red-50 rounded-full transition-colors opacity-0 group-hover:opacity-100 cursor-pointer"
                                        title="Descartar"
                                    >
                                    ...
                                    {!candidate.candidate_epmi ? (
                                        ...
                                        <div className="bg-blue-50 p-3 rounded-xl mb-4 text-xs text-blue-700 font-bold flex items-center">
                                    ...
                                    <div className="flex gap-2 mt-auto">
                                        {!candidate.candidate_epmi ? (
                                            ...
                                            <button
                                                ...
                                                className="flex-1 py-3 bg-slate-900 text-white font-bold rounded-xl hover:bg-black transition-all shadow-md shadow-slate-300 flex justify-center items-center gap-2 cursor-pointer"
                                            >
                ...
                <div className="space-y-6">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        {serviceStudents.map(student => {
                            ...
                            return (
                                <div key={student.id} className="bg-white p-6 rounded-2xl shadow-sm border border-slate-100 relative group hover:shadow-lg transition-all">
            ...
            {activeTab === 'GRADUADOS' && (
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    {graduatedStudents.map(student => {
                        const member = members.find(m => m.id === student.memberId);
                        return (
                            <div key={student.id} className="bg-white p-6 rounded-2xl shadow-sm border border-slate-100 flex items-center justify-between hover:shadow-lg transition-all">
            ...
            {isManualSearchOpen && (
                <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/40 backdrop-blur-sm p-4 animate-fadeIn">
                    <div className="bg-white w-full max-w-md rounded-[2rem] shadow-2xl p-8 relative border border-white/50 h-[60vh] flex flex-col">
                        <button onClick={() => setIsManualSearchOpen(false)} className="absolute top-6 right-6 p-2 bg-slate-50 rounded-full hover:bg-slate-100 text-slate-400 cursor-pointer"><X className="w-5 h-5" /></button>
                        <h3 className="text-xl font-extrabold text-slate-800 mb-4">Postular Miembro</h3>

                        <div className="relative mb-4">
                            <input
                                className="w-full pl-10 pr-4 py-3 bg-slate-50 rounded-xl border-none focus:ring-2 focus:ring-blue-200 font-medium outline-none"
                        ...
                        <div className="flex-1 overflow-y-auto space-y-2 pr-2 custom-scrollbar">
                            {members.filter(m =>
                                ...
                                <div key={m.id} className="flex items-center justify-between p-3 border border-slate-100 rounded-xl hover:bg-blue-50 cursor-pointer transition-colors" onClick={() => handleManualPostulation(m.id)}>
                                    <div className="flex items-center gap-3">
            ...
            {isServiceAssignOpen && selectedStudent && (
                <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/40 backdrop-blur-sm p-4 animate-fadeIn">
                    <div className="bg-white w-full max-w-sm rounded-[2rem] shadow-2xl p-8 relative border border-white/50">
                        <button onClick={() => setIsServiceAssignOpen(false)} className="absolute top-6 right-6 p-2 bg-slate-50 rounded-full hover:bg-slate-100 text-slate-400 cursor-pointer"><X className="w-5 h-5" /></button>
                        <h3 className="text-xl font-extrabold text-slate-800 mb-6">Asignación Práctica</h3>
                        ...
                        <div className="space-y-4">
                            <div>
                                ...
                                <input
                                    ...
                                    className="w-full p-3 bg-slate-50 rounded-xl border border-slate-200 outline-none focus:ring-2 focus:ring-blue-100 font-bold text-slate-700 transition-all"
                            ...
                            <div>
                                ...
                                <select
                                    className="w-full p-3 bg-slate-50 rounded-xl border border-slate-200 outline-none focus:ring-2 focus:ring-blue-100 font-medium text-slate-700 transition-all"
                                    value={serviceSupervisorId}
                                    onChange={e => setServiceSupervisorId(e.target.value)}
                                >
                                    <option value="">-- Seleccionar --</option>
                                    {members.filter(m => ['PASTOR_PRINCIPAL', 'PASTOR_GENERAL', 'PASTORA_GENERAL', 'MINISTRO', 'LIDER_ANEXO'].includes(m.role || 'MIEMBRO')).map(m => (
                            ...
                            <button
                                ...
                                className="w-full py-3 bg-slate-900 text-white rounded-xl font-bold hover:bg-black transition-all shadow-lg shadow-slate-300 mt-4 disabled:opacity-50 cursor-pointer"
                            >
            ...
            {isEditStudentOpen && selectedStudent && (
                <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/40 backdrop-blur-sm p-4 animate-fadeIn">
                    <div className="bg-white w-full max-w-sm rounded-[2rem] shadow-2xl p-8 relative border border-white/50">
                        <button onClick={() => setIsEditStudentOpen(false)} className="absolute top-6 right-6 p-2 bg-slate-50 rounded-full hover:bg-slate-100 text-slate-400 cursor-pointer"><X className="w-5 h-5" /></button>
                        <h3 className="text-xl font-extrabold text-slate-800 mb-6">Editar Estudiante</h3>
                        ...
                        <div className="space-y-4">
                            ...
                            <button
                                ...
                                className="w-full py-3 bg-slate-900 text-white rounded-xl font-bold hover:bg-black transition-colors shadow-lg mt-4 cursor-pointer"
                            >
                                Guardar Cambios
                            </button>
                        </div>
            ...
            {isGradingOpen && selectedStudent && (
                <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/40 backdrop-blur-sm p-4 animate-fadeIn">
                    <div className="bg-white w-full max-w-sm rounded-[2rem] shadow-2xl p-8 relative border border-white/50">
                        <button onClick={() => setIsGradingOpen(false)} className="absolute top-6 right-6 p-2 bg-slate-50 rounded-full hover:bg-slate-100 text-slate-400 cursor-pointer"><X className="w-5 h-5" /></button>
                        <h3 className="text-xl font-extrabold text-slate-800 mb-6">Registrar Calificación</h3>
                        ...
                        <form onSubmit={handleSubmitGrade} className="space-y-4">
                            ...
                            <button type="submit" className="w-full py-3 bg-blue-600 text-white rounded-2xl font-bold hover:bg-blue-700 transition-colors shadow-lg flex justify-center items-center gap-2 mt-2 cursor-pointer">
                ...
                <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/40 backdrop-blur-sm p-4 animate-fadeIn">
                    <div className="bg-white w-full max-w-sm rounded-[2rem] shadow-2xl p-8 relative border border-white/50 text-center">
                        ...
                        <div className="flex gap-3">
                            <button onClick={() => setPromoteStudent(null)} className="flex-1 py-3 bg-slate-100 text-slate-600 font-bold rounded-xl hover:bg-slate-200 cursor-pointer">Cancelar</button>
                            <button onClick={handleConfirmPromote} className="flex-1 py-3 bg-blue-600 text-white font-bold rounded-xl hover:bg-blue-700 shadow-lg cursor-pointer">Confirmar</button>
                        </div>
                    </div>
                </div>
            )}
            ...
            {graduateStudent && (
                <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/40 backdrop-blur-sm p-4 animate-fadeIn">
                    <div className="bg-white w-full max-w-sm rounded-[2rem] shadow-2xl p-8 relative border border-white/50 text-center">
                        ...
                        <div className="flex gap-3">
                            <button onClick={() => setGraduateStudent(null)} className="flex-1 py-3 bg-slate-100 text-slate-600 font-bold rounded-xl hover:bg-slate-200 cursor-pointer">Cancelar</button>
                            <button onClick={handleGraduate} className="flex-1 py-3 bg-emerald-600 text-white font-bold rounded-xl hover:bg-emerald-700 shadow-lg cursor-pointer">Graduar</button>
                        </div>
                    </div>
                </div>
            )}
            ...
            {certificateStudent && (
                <div className="fixed inset-0 z-[70] flex items-center justify-center bg-slate-900/80 backdrop-blur-md p-4 animate-fadeIn overflow-auto print-only-visible">
                    <div className="bg-[#fffdf5] w-full max-w-4xl aspect-[1.414/1] shadow-2xl relative border-[16px] border-double border-[#b49b57] p-12 flex flex-col items-center justify-between text-center print:shadow-none print:w-full print:h-full print:absolute print:inset-0 print:m-0 print:border-[10px]">
                        ...
                        <button onClick={() => setCertificateStudent(null)} className="absolute top-4 right-4 p-2 bg-black/10 rounded-full hover:bg-black/20 text-slate-800 no-print cursor-pointer"><X className="w-6 h-6" /></button>
                        <button onClick={() => window.print()} className="absolute top-4 right-16 p-2 bg-black/10 rounded-full hover:bg-black/20 text-slate-800 no-print cursor-pointer"><Printer className="w-6 h-6" /></button>

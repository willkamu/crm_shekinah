  ...
  return (
    <div className="space-y-6 animate-fadeIn">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        ...
        <div className="flex items-center gap-3 bg-white p-1.5 rounded-2xl shadow-sm border border-slate-100">
            <button 
                ...
                className={`p-2 rounded-xl transition-colors ${viewMode === 'LIST' ? 'bg-sky-50 text-sky-600' : 'text-slate-400 hover:text-slate-600'}`}
            >
                <List className="w-5 h-5" />
            </button>
            <button 
                onClick={() => setViewMode('CALENDAR')}
                className={`p-2 rounded-xl transition-colors ${viewMode === 'CALENDAR' ? 'bg-sky-50 text-sky-600' : 'text-slate-400 hover:text-slate-600'}`}
            >
            ...
            <button 
                ...
                className="bg-sky-600 text-white px-4 py-2 rounded-xl flex items-center shadow-glow hover:bg-sky-700 transition-all text-xs font-bold"
            >
      ...
      {viewMode === 'LIST' ? (
                    ...
                    <div key={evt.id} className={`p-5 flex flex-col sm:flex-row items-start sm:items-center gap-4 hover:bg-slate-50 transition-colors ${idx !== events.length-1 ? 'border-b border-slate-100' : ''}`}>
                        ...
                        <div className="flex row sm:flex-col items-center gap-3 sm:gap-0 bg-slate-50 sm:bg-transparent p-2 sm:p-0 rounded-xl w-full sm:w-auto">
                            <div className="flex flex-col items-center bg-white sm:bg-slate-100 p-3 rounded-2xl min-w-[70px] border border-slate-100 sm:border-transparent shadow-sm sm:shadow-none">
                                <span className="text-[10px] font-extrabold text-red-500 uppercase tracking-wider">{new Date(evt.fecha).toLocaleString('es-ES', { month: 'short' }).replace('.', '')}</span>
                            ...
                            {evt.estado === 'PENDIENTE' ? (
                                <div className="flex items-center gap-2">
                                    ...
                                    {currentUser.role === 'PASTOR_PRINCIPAL' && (
                                        <button onClick={() => setEventToApprove(evt.id)} className="px-4 py-1.5 bg-emerald-500 text-white rounded-xl text-xs font-bold hover:bg-emerald-600 shadow-glow transition-transform hover:scale-105">
                                            Aprobar
                                        </button>
                                    )}
                                </div>
          ...
          <div className="bg-white rounded-[2.5rem] border border-slate-200 overflow-hidden shadow-card p-6">
              {/* Calendar Header */}
              <div className="flex justify-between items-center mb-6">
                  <button onClick={() => changeMonth(-1)} className="p-2 bg-slate-50 rounded-full hover:bg-slate-100"><ChevronLeft className="w-5 h-5 text-slate-600"/></button>
                  <h3 className="text-xl font-bold text-slate-800 capitalize">{monthName}</h3>
                  <button onClick={() => changeMonth(1)} className="p-2 bg-slate-50 rounded-full hover:bg-slate-100"><ChevronRight className="w-5 h-5 text-slate-600"/></button>
              </div>
              ...
              <div className="grid grid-cols-7 gap-2">
                  ...
                  {calendarDays.map((date, i) => {
                      ...
                      return (
                          <div key={i} className={`h-28 border border-slate-100 rounded-xl p-2 flex flex-col gap-1 transition-colors hover:border-sky-200 ${isToday ? 'bg-sky-50/30 ring-2 ring-sky-100' : 'bg-white'}`}>
                              <span className={`text-xs font-bold mb-1 ${isToday ? 'text-sky-600' : 'text-slate-400'}`}>{date.getDate()}</span>
                              <div className="flex-1 overflow-y-auto custom-scrollbar space-y-1">
                                  {dayEvents.map(ev => (
                                      <div key={ev.id} className={`text-[10px] px-1.5 py-1 rounded font-bold truncate cursor-pointer hover:opacity-80 ${ev.estado === 'PENDIENTE' ? 'bg-amber-100 text-amber-700' : 'bg-sky-100 text-sky-700'}`} title={ev.nombre}>
                                          {ev.nombre}
                                      </div>
                                  ))}
                              </div>
          ...
          <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/40 backdrop-blur-sm p-4 animate-fadeIn">
              <div className="bg-white w-full max-w-md rounded-[2.5rem] shadow-2xl p-8 relative border border-white/50">
                  <button 
                    ...
                    className="absolute top-6 right-6 p-2 bg-slate-50 rounded-full hover:bg-slate-100 text-slate-400 transition-colors"
                  >
                      <X className="w-5 h-5" />
                  </button>
                  ...
                  <form onSubmit={handleRequestActivity} className="space-y-4">
                      <div>
                          ...
                          <input 
                            ...
                            className="w-full p-4 bg-slate-50 rounded-2xl border border-slate-100 font-bold text-slate-700 outline-none focus:ring-2 focus:ring-sky-200 transition-all"
                            autoFocus
                          />
                      </div>
                          ...
                          <div>
                              ...
                              <div className="relative">
                                <input 
                                    ...
                                    className="w-full p-4 bg-slate-50 rounded-2xl border border-slate-100 font-medium text-slate-700 outline-none focus:ring-2 focus:ring-sky-200"
                                />
                              </div>
                          </div>
                          <div>
                              ...
                              <select 
                                ...
                                className="w-full p-4 bg-slate-50 rounded-2xl border border-slate-100 font-medium text-slate-700 outline-none focus:ring-2 focus:ring-sky-200 appearance-none"
                              >
                      ...
                      <div className="pt-2">
                          <button type="submit" className="w-full py-4 bg-sky-600 text-white rounded-2xl font-bold shadow-lg hover:bg-sky-700 transition-transform active:scale-95 flex justify-center gap-2">
          ...
          <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/40 backdrop-blur-sm p-4 animate-fadeIn">
              <div className="bg-white w-full max-w-sm rounded-[2.5rem] shadow-2xl p-8 relative border border-white/50 text-center">
                  ...
                  <div className="flex gap-3">
                      <button onClick={() => setEventToApprove(null)} className="flex-1 py-3 bg-slate-100 text-slate-600 font-bold rounded-xl hover:bg-slate-200">Cancelar</button>
                      <button onClick={confirmApprove} className="flex-1 py-3 bg-emerald-500 text-white font-bold rounded-xl hover:bg-emerald-600 shadow-lg">Aprobar</button>
                  </div>
              </div>
          </div>
      )}
